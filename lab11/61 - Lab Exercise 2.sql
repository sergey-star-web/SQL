...
--Filter the results to include only the rows in which the pricetype column value is equal to high. Use 
--the SELECT statement from exercise 1, task 4 as the inner query that defines the derived table. 
...
--which returns the average sales amount of orders per customer. To correctly calculate the
--average sales amount of orders per customer, you will first have to calculate the total 
--sales amount per order. You can do so by defining a derived table based on a query that 
--joins the Sales.Orders and Sales.OrderDetails tables. You can use the custid and orderid
--columns from the Sales.Orders table and the qty and unitprice columns from the Sales.OrderDetails table.
--
...
--  percentgrowth, representing the percentage of sales growth in the current order year compared
--to the previous order year 
-- You will have to write a T-SQL statement using two derived tables. To get the order year and 
--total sales columns for each SELECT statement, you can query an already existing view named 
--Sales.OrderValues.
--The val column represents the sales amount.
...
---------------------------------------------------------------------
select t1.orderyear, t1.curtotalsales, t1.prevtotalsales, 
(t1.curtotalsales-t1.prevtotalsales)*100/t1.prevtotalsales as percentgrowth from(
select year(ov.orderdate) as orderyear, sum(ov.val) as curtotalsales,
lag(sum(ov.val),1) over (order by year(ov.orderdate)) as prevtotalsales
from Sales.OrderValues as ov
group by year(ov.orderdate)) as t1